...
Akshit Gandotra
Question 1 (Lab)
Implement single Linked List data structure and its operations like insert and delete in the beginning/end and nth position
...
void createList()
{
	if (start == NULL) {
		...
		if (n != 0) {
			...
			struct node* newnode;
			struct node* temp;
			newnode = malloc(sizeof(struct node));  // dynamic allocation
			start = newnode;
			temp = start;
	...
	// Else
		...
		while (current != NULL) {
			...
			while (index != NULL) {
				...
				// Swap with it the value at current
